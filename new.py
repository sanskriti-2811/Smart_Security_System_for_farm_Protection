...
intruder_body = 'An intruder has been detected by the security camera.'
owner_body = 'The owner is present.'
...
owner_detected = False  # Flag to track owner detection
...
while True:
    ...
    if len(results.pred) > 0:
        if not owner_detected:
            ...
            em['Subject'] = subject
            em.attach(MIMEText(intruder_body))
